...
* `Exclude QR Code from Selected Tickets` - Plugin Human-readable name, e.g. "Sample Extension"
* `Exclude_QR_Code_From_Selected_Tickets` - Which namespace we will use for the plugin. Usually the plugin name or the slug in uppercase without spaces, e.g. "SampleExtension"
* `exclude-qr-code-from-selected-tickets` - Uses using dashes normally, e.g. "sample-extension"
* `__TRIBE_DOMAIN__` - Translation domain normally with "tribe-" prefix, e.g. "tec-labs-sample-extension"
* `exclude_qr_code_from_selected_tickets` - Uses underscores, so it's safe for variables, e.g. "sample_extension"
